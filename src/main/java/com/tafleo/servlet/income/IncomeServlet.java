...
public class IncomeServlet extends HttpServlet implements Common {
    ...
    public void selectFunction(HttpServletRequest req, HttpServletResponse resp) {
        ...
        } else if ("save".equals(method) && method != null) {
            this.save(req, resp);
        }
    }
    ...
    public void countPrice(HttpServletRequest req, HttpServletResponse resp) {
        ...
        income.setDailyIncome(sumPrice);
        ...
        int price = oldPrice + income.getDailyIncome();
    ...
    public void save(HttpServletRequest req, HttpServletResponse resp) {
        ...
        Income income = incomeService.getIncome();
        int oldPrice = income.getTotalIncome();
        int dailyPrice = income.getDailyIncome();
        income.setTotalIncome(oldPrice + dailyPrice);
        String time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date());
        Date date = null;
        try {
            date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(time);
        ...
        try {
            PrintWriter out = resp.getWriter();
            if (incomeService.modify(income)) {
                out.write("true");
            } else {
                out.write("false");
            }
